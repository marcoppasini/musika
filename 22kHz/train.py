...
class Train_functions:
    ...
    def train_all(self, a, ema, g_train=True, disc_train=True, models_ls=None):
        ...
        noiseg = tf.random.normal([self.args.bs, 64], dtype=tf.float32)

        noisel = tf.concat([tf.random.normal([self.args.bs, 64], dtype=tf.float32), noiseg], -1)
        noisec = tf.concat([tf.random.normal([self.args.bs, 64], dtype=tf.float32), noiseg], -1)
        noiser = tf.concat([tf.random.normal([self.args.bs, 64], dtype=tf.float32), noiseg], -1)

        rl = tf.linspace(noisel, noisec, self.args.latlen, axis=-2)[:, :-1, :]
        rr = tf.linspace(noisec, noiser, self.args.latlen, axis=-2)

        noisetot = tf.concat([rl, rr], -2)
        noise = tf.image.random_crop(noisetot, [self.args.bs, self.args.latlen, 64 + 64])

        with tf.GradientTape() as tape_dec, tf.GradientTape() as tape_disc:
            if not disc_train:
                tape_disc.stop_recording()
            if not g_train:
                tape_dec.stop_recording()
            ...
            if disc_train or g_train:
                ...
                c, cr = critic(tf.concat([a, ab], 0), training=True)

                c = tf.where(tf.math.is_nan(c), 0.0, c)
                cr = tf.where(tf.math.is_nan(cr), 0.0, cr)

                ca, cab = c[: self.args.bs], c[self.args.bs :]
                car, _ = cr[: self.args.bs], cr[self.args.bs :]

                cabr = critic_rec(car, training=True)
            ...
            if disc_train:
                ...
                loss_did = msesum(a, cabr)

                loss_gp = self.gradient_penalty(a, critic)

                loss_d = loss_dt + 10.0 * loss_gp + 10.0 * loss_did
            ...
            if g_train:
                ...
                if self.args.mixed_precision:
                    loss_dec = opt_dec.get_scaled_loss(loss_dec)

        if disc_train:
            grad_disc = tape_disc.gradient(loss_d, critic.trainable_weights + critic_rec.trainable_weights)
            if self.args.mixed_precision:
                grad_disc = opt_disc.get_unscaled_gradients(grad_disc)
            opt_disc.apply_gradients(zip(grad_disc, critic.trainable_weights + critic_rec.trainable_weights))

        if g_train:
            grad_dec = tape_dec.gradient(loss_dec, gen.trainable_variables)
            if self.args.mixed_precision:
        ...
        return loss_dtr, loss_dtf, loss_gp, loss_gt, loss_did
    ...
    def train(self, ds, models_ls=None):
        ...
        for epoch in range(self.args.epochs):
            ...
            for batchi, (wv) in enumerate(pbar):
                ...
                dloss_tr, dloss_tf, dloss_id, gloss_t, idloss = train_tot(a, ema, models_ls=models_ls)
                ...
                with train_summary_writer.as_default():
                    ...
                    tf.summary.scalar("rec_loss", idloss, step=m)
